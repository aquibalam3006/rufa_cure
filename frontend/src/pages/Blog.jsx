    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800 flex flex-col">
      ...
      <div className="bg-gradient-to-r from-[#008985] to-[#005a57] py-16 px-8 text-center text-white relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          <span className="text-teal-200 font-extrabold tracking-wider uppercase text-sm mb-4 block">RuFa Cure Insights</span>
          <h1 className="text-4xl md:text-5xl font-extrabold mb-4 leading-tight">
            Health & Wellness Blog
          </h1>
          <p className="text-lg text-teal-100 font-medium leading-relaxed max-w-2xl mx-auto">
      ...
      {/* --- CATEGORY FILTERS --- */}
      <div className="max-w-7xl mx-auto px-8 w-full -mt-6 relative z-10 mb-12">
        <div className="bg-white p-2 rounded-2xl shadow-md border border-gray-100 flex overflow-x-auto hide-scrollbar">
          <div className="flex space-x-2 w-full justify-start md:justify-center min-w-max px-2">
      ...
      {/* --- MAIN BLOG CONTENT --- */}
      <div className="max-w-7xl mx-auto px-8 pb-20 w-full flex-1">
        
        {/* Featured Post (Only shows when 'All' is selected) */}
        {activeCategory === 'All' && featuredPost && (
          <div className="mb-16 bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col md:flex-row hover:shadow-xl transition duration-300 group cursor-pointer">
            <div className={`w-full md:w-1/2 min-h-[300px] ${featuredPost.imageColor} relative overflow-hidden flex items-center justify-center`}>
              <span className="text-6xl group-hover:scale-110 transition duration-500">🩺</span>
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm text-[#008985] text-xs font-extrabold px-3 py-1 rounded-full uppercase tracking-wider">
                Featured
              </div>
            </div>
            <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
              <span className="text-[#008985] font-extrabold text-sm mb-3 tracking-wider uppercase">{featuredPost.category}</span>
              <h2 className="text-3xl font-extrabold text-gray-900 mb-4 leading-tight group-hover:text-[#008985] transition">
                {featuredPost.title}
              </h2>
              ...
              <div className="mt-auto flex items-center justify-between pt-6 border-t border-gray-100">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#008985] text-white flex items-center justify-center font-bold shadow-sm">
                ...
                <button className="w-10 h-10 rounded-full bg-gray-50 text-[#008985] flex items-center justify-center group-hover:bg-[#008985] group-hover:text-white transition shadow-sm">
                  ➔
                </button>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredPosts.length > 0 ? (
            filteredPosts.map((post) => (
              <div key={post.id} className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition duration-300 group cursor-pointer flex flex-col h-full">
                
                {/* 🚀 YAHAN IMAGES FIT KI GAYI HAIN */}
                <div className={`w-full h-48 ${!post.image ? post.imageColor : 'bg-gray-100'} relative flex items-center justify-center overflow-hidden`}>
                  {post.image ? (
                    <img 
                      src={post.image} 
                      alt={post.title} 
                    ...
                    <span className="text-5xl opacity-50 group-hover:scale-110 transition duration-500">📄</span>
                  )}
                  
                  <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm text-gray-600 text-xs font-extrabold px-3 py-1 rounded-full">
                    {post.readTime}
                  </div>
                </div>

                <div className="p-6 flex flex-col flex-1">
                  <span className="text-[#008985] font-extrabold text-xs mb-3 tracking-wider uppercase">{post.category}</span>
                  <h3 className="text-xl font-extrabold text-gray-900 mb-3 leading-snug group-hover:text-[#008985] transition">
                    {post.title}
                  </h3>
                  <p className="text-gray-600 font-medium text-sm leading-relaxed mb-6 flex-1">
                    {post.excerpt}
                  </p>
                  <div className="mt-auto flex items-center gap-3 pt-5 border-t border-gray-50">
                    ...
                    <div>
                      <p className="font-bold text-gray-900 text-xs">{post.author}</p>
                      <p className="text-[10px] text-gray-500">{post.authorRole} • {post.date}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="bg-[#e6f4f4] py-16 px-8 mt-auto">
        <div className="max-w-4xl mx-auto text-center">
          <span className="text-4xl mb-4 block">💌</span>
          <h2 className="text-3xl font-extrabold text-gray-900 mb-4">Subscribe to our Health Newsletter</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input 
              ...
              className="flex-1 px-5 py-3.5 rounded-xl border border-gray-200 outline-none focus:border-[#008985] text-sm font-medium shadow-sm"
            />
            <button className="bg-[#008985] text-white px-8 py-3.5 rounded-xl font-bold hover:bg-[#005a57] transition shadow-md whitespace-nowrap">
              Subscribe Now
            </button>
          </div>
